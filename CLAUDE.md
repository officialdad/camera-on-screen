...
Config lands at `$XDG_CONFIG_HOME/CameraOnScreen/config.json` (Windows keeps `%LOCALAPPDATA%`). Linux CI: `.github/workflows/ci-linux.yml` — hosted `linux` job (no GPU/camera — stub shim; `v4l2_probe` passes with 0 cameras by design) + `maxine-rtx` job on the dev box's self-hosted `[self-hosted, linux, rtx]` runner (`cachyos-rtx3090`, user systemd service; SDK-config build, deploy-check, GPU-only capability probe — runbook: `docs/ci/self-hosted-runner.md`). **Releases: a `v*` tag runs `release.yml`'s `release-linux` job on the same runner** — `publish-linux.sh --tar`, bundled-tier probe gate (`COS_*` unset), tarball uploaded as the GitHub release. Windows Maxine CI + installer releases still need a Windows RTX box (#38). V4L2 format support is XBGR32/BGR24/RGB24/YUYV (fps-first scoring, ≤1080p) — MJPEG-only high-res modes (e.g. Brio 100 1080p30) fall back to lower-res YUYV until a libjpeg decode path is added.
