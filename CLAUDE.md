...
4. **QoL Bucket 3 — green-screen Expand + Feather** (NEXT). Two sliders to fix the matte cutting into the body. **Shim + C-ABI work:** repurpose the reserved-but-unused `green_screen_strength` → `green_screen_expand` (matte **dilate**, grows coverage outward) and **add** a new `green_screen_feather` field (matte **blur**, soft edge) — one new `double` across all 6 ABI parity sites (`shim.h` `CosParams`, `PInvokeShim`, `Contracts`, `Models`, `MainViewModel`, XAML), byte-parity on x64. Matte ops live in `aigs.cpp` `Composite()`, order **dilate → feather → premultiply composite**, worker-thread-local, pushed live via the atomic param path. Needs the **SDK shim rebuilt (SDK config LAST)** + a visual gate on the RTX 3090. Spec: the QoL polish design doc (Bucket 3 section).
5. **QoL Bucket 1 — control panel right-size.** Panel opens at the WinUI default (~1100×700) for a few rows of controls. `AppWindow.Resize(...)` to a compact size in the `MainWindow` ctor; no redesign. Spec: QoL polish design (Bucket 1 section).
6. **M4 — Eye Contact.** Needs the separate **Maxine AR SDK** installed (NOT the VFX SDK) + a new effect in the chain; brainstorm → spec → plan as its own milestone.
7. **M5 — ship-time** (the Distribution bullet above): bundle runtime+models, app-relative SDK discovery, multi-GPU models, license compliance, installer.
