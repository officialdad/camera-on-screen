...
- **A native probe gates effects (not the GPU tier).** The `Orchestrator` sets `EffectsAvailable` from `INativeShim.QueryCapabilities()` → `cos_query_capabilities`, which actually tries to create+load the Maxine GreenScreen effect. `GpuTierDetector.Detect()` (Vortice DXGI, "RTX" substring) is now **display-only** (GPU name string), NOT the gate. No SDK / effect-load failure → toggles disabled, passthrough (no crash). The disabled-state note currently shows a stale static "requires RTX GPU" string — a logged follow-up is to bind it to the probe's `CapabilityDetail`.
...
Ordered roughly by size. The two M3 polish items are small and live in the same area (`Orchestrator` ctor + the WinUI control-panel note binding):

1. **Surface the real probe reason.** When effects are unavailable the control panel shows a static "Effects require an RTX GPU" string. Bind that note to the `Orchestrator.CapabilityDetail` (already populated from `cos_query_capabilities`) so the actual reason shows (e.g. "Maxine SDK not built in", "COS_VFX_SDK_DIR not set"). This stale message masked a real bug once.
2. **Make the capability probe async/lazy.** `Orchestrator` calls `QueryCapabilities()` synchronously in its constructor; the real probe does a TensorRT model load (~1s) on the UI thread → visible startup freeze. Run it on a background task and raise `PropertyChanged` on `EffectsAvailable`/`CapabilityDetail`, or make it lazy on first toggle.
