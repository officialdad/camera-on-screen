...
5. **QoL Bucket 1 — control panel right-size** (NEXT). Panel opens at the WinUI default (~1100×700) for a few rows of controls. `AppWindow.Resize(...)` to a compact size in the `MainWindow` ctor; no redesign. Spec: QoL polish design (Bucket 1 section).
