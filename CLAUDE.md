...
M1 (Core), M2 (App + shim + overlay passthrough), M3 (AI Green Screen), the 3 QoL buckets, and **M4 (AI Eye Contact / gaze redirection)** are all complete and merged to `main`. **M4 is user-verified on an RTX 3090 (`5be7619`): green screen + eye contact work together and alone, live toggles work.** M4 required a **co-version fix** — two Maxine SDKs with mismatched CUDA/TensorRT can't share one process; pinned VFX **0.7.6** + AR **0.8.7** onto shared **TensorRT 10.4** via `COS_VFX_RUNTIME_DIR` (see the CO-VERSION GOTCHA in Build & test, and follow-up #6). **Next: M5 — ship-time.**
...
7. **M5 — ship-time (NEXT)** (the Distribution bullet above): bundle the **co-versioned** runtime+models for BOTH SDKs (VFX 0.7.6 + AR 0.8.7, shared TRT 10.4), app-relative SDK discovery (drop `COS_VFX_RUNTIME_DIR`/`COS_AR_RUNTIME_DIR`/`COS_*_SDK_DIR`), multi-GPU models (compute 86 only today), license compliance (Maxine EULA PDFs in each SDK), installer. The co-version constraint is now a hard ship requirement.
