...
| 11 | Launch and quit the app five times in a row | The panel window appears every time. This fix wave added an `_opened` latch on the minimize-to-tray handler to fix a suspected transient-`WindowState`-during-map cause (an icon-but-no-window launch was seen once during development) — re-run this row to confirm the fix, not merely to watch for a recurrence |
